...
# Contextual ad queries per post — tuned for relevant ZeroClick offers
AD_QUERIES = {
    "the-next-eight-billion": "AI autonomous agents business infrastructure cloud computing",
    "the-protocol-stack": "API developer tools blockchain payments infrastructure",
    "trust-at-machine-speed": "cybersecurity identity verification trust platform",
    "hayek-coase-and-homo-agenticus": "economics AI business strategy automation",
    "what-happens-next": "AI governance risk management enterprise software",
}
...
def render_blog_post(slug: str) -> str | None:
    ...
    # Build ad query from post topic
    ad_query = AD_QUERIES.get(post["slug"], post["title"])
    ...
    result = result.replace("{{ad_query}}", ad_query)
...
(function() {{
  ...
  async function loadOffers() {{
    try {{
      ...
      // Track impressions (must come from client device per ZeroClick docs)
      ...
      // Silently fail — ads are non-essential
    }}
  }}
